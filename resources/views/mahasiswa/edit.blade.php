<form action="/mahasiswa/{{ $mahasiswa->nim }}/update" method="POST" id="formmahasiswa" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-12">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <select name="kode_kls" id="kode_kls" class="form-select">
                ...
                @foreach ($kelas as $d)
                    <option {{ $mahasiswa->kode_kls == $d->kode_kls ? 'selected' : '' }} value="{{ $d->kode_kls }}">{{ $d->kls }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="row mt-1">
        <div class="col-12">
            <input type="file" value="{{ $mahasiswa->foto }}" name="foto" class="form-control">
